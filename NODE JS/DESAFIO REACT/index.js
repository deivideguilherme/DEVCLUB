...
const checkIdPedido = (request, response, next) => {
    ...
    if (index < 0) {
        ...
        return response.status(404).json({ error: 'Pedido não encointrado'})
    }
...
app.post('/pedidos', (request, response) => {
    ...
    const novoPedido = { id: uuid.v4(), pedido, nomeCliente, preco }
...
app.put('/pedidos/:id', checkIdPedido, (request, response) => {
    ...
    const atualizaPedido = { id, pedido, nomeCliente, preco }
